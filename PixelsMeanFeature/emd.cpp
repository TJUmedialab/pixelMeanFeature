...
******************************************************************************/
EMD::EMD(signature_t *Signature1, signature_t *Signature2,
		 ...
		 flow_t *Flow, int *FlowSize)
{
	...
	_RowsX = new node2_t *[_n1+1];
	_ColsX = new node2_t *[_n2+1];
	U = new node1_t[_n1];
	V = new node1_t[_n2];
	_C = new float *[_n1+1];
	for ( int i = 0 ; i < _n1 + 1; i ++) {
		_C[i] = new float[_n2 + 1] ;
	}
	_IsX = new char *[_n1+1];
	for ( int i = 0 ; i < _n1 + 1; i ++) {
		_IsX[i] = new char[_n2 + 1] ;
	}
...
/**********************
	 init
**********************/
float EMD::init(signature_t *Signature1, signature_t *Signature2, 
			float (*Dist)(feature_t *, feature_t *))
{
	int i, j;
	double sSum, dSum, diff;
	feature_t *P1, *P2;
	double *S = new double[Signature1->n + 1], *D= new double[Signature2->n + 1];
...
/**********************
		findLoop
**********************/
int EMD::findLoop(node2_t **Loop)
{
	int i, steps;
	node2_t **CurX, *NewX;
	char *IsUsed = new char[_n1+_n2]; 
...
void EMD::russel(double *S, double *D)
{
	...
	double **Delta;
	node1_t *Ur = new node1_t[_n1], *Vr = new node1_t[_n2];
	node1_t uHead, *CurU, *PrevU;
	node1_t vHead, *CurV, *PrevV;
	node1_t *PrevUMinI, *PrevVMinJ, *Remember;

	Delta = new double *[_n1];
	for ( i = 0 ; i < _n1 ; i ++) {
		Delta[i] = new double[_n2];
	}
