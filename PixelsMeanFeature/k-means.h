...
public:
	...
	*/
	...
	void Cluster(double *data, int N, double *Label);
...
private:
	...
	double GetLabel(double* x, int* label);
	double EMDDistance(double* x, const double* u, int dimNum);
	float CalcDistance(feature_t *F1, feature_t *F2);
	double CalcDistance(double* x, const double* u, int dimNum);
};
