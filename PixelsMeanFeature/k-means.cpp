...
History:
	2003/10/16	Fei Wang
	2013 luxiaoxun
	2013/11/18 Yifeng Zhang : added Earth Mover's Distance(EMD)
***************************************************************************/
...
//N 为特征向量数
void KMeans::Cluster(double *data, int N, double *centre)
{
	int size = 0;
	size = N;
 ...
 //           x[j] = data[i*m_dimNum+j];
	...
	//	Label[i] = label;
	//}
	memcpy(centre, m_means[0], sizeof(double) * m_dimNum);
	delete[] counts;
	delete[] x;
	for(int i = 0; i < m_clusterNum; i++)
	{
...
double KMeans::GetLabel(double* sample, int* label)
{
	double dist = -1;
	for(int i = 0; i < m_clusterNum; i++)
	{
#ifdef USEEMD
		double temp = EMDDistance(sample, m_means[i], m_dimNum);
#else
...
double KMeans::CalcDistance(double* x, const double* u, int dimNum)
{
	double temp = 0;
...
float KMeans:: CalcDistance(feature_t *F1, feature_t *F2) { 
	return (*F1-*F2)*(*F1-*F2); 
}

double KMeans::EMDDistance(double* x, const double* u, int dimNum)
{
	feature_t *position_x = new feature_t[dimNum];
	feature_t *position_u = new feature_t[dimNum];

	signature_t signature_tx = {dimNum, position_x, x};
